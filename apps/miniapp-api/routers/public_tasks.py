...
def list_public_tasks(
    ...
    if not dbid:
        logger.warning("Notion public tasks DB is not configured; returning cached fallback (empty).")
        _mark_state(request, False)
        return []
    try:
        client = _client()
    except ValueError:
        logger.warning("Notion credentials are missing; returning empty public tasks list.")
        _mark_state(request, False)
        return []
